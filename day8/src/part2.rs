...
fn calculate_antinodes(sig1: &Coord, sig2: &Coord, size: &Size) -> HashSet<Coord> {
    ...
    antinodes.insert(*sig1);
    antinodes.insert(*sig2);

    let node_vec = *sig2 - *sig1;
    let mut node1 = *sig1 - node_vec;
    while is_on_map(&node1, size) {
        antinodes.insert(node1);
        node1 = node1 - node_vec;
    }

    let mut node2 = *sig2 + node_vec;
    while is_on_map(&node2, size) {
        antinodes.insert(node2);
        node2 = node2 + node_vec;
    }
    antinodes
}
...
mod tests {
    ...
    fn test_part2() {
        ...
        assert_eq!(run(&example_content), 34);
    }
