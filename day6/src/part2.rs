...
fn walk(
    ...
    while is_on_map(&cur_pos, map_size) {
        steps.insert((cur_pos, cur_dir));
        let new_pos = cur_pos + cur_dir;
        if place_obstacle && &new_pos != start_pos {
            let mut new_obstacles = obstacles.clone();
            new_obstacles.insert(new_pos);
            let loop_count = walk(start_pos, &new_obstacles, map_size, false);
            if loop_count > 0 {
                placed_obstacles.insert(new_pos);
            }
        } else if steps.contains(&(new_pos, cur_dir)) {
            nr_loops += 1;
            break;
        }
...
pub fn run(input: &str) -> usize {
    ...
    walk(&start_pos, &obstacles, &map_size, true)
}
